...
public class MessageDataSource {
	...
	public long insertMessage(String school, Element message) {
		...
		initialValues.put(MessageHelper.COLUMN_DID_READ, "0");
	...
	public long insertMessages(String school, Element content) {
		long id = -1;
		for (Element element : XmlUtil.selectElements(content, "Message"))
			id = insertMessage(school, element);
	 ...
	 *            : 指定學校
	 * **/
	public long getLastUid(String school) {
		Cursor mCursor = database.query(MessageHelper.TABLE_MESSAGE,
				new String[] { MessageHelper.COLUMN_SERVER_UID },
				MessageHelper.COLUMN_SERVER + "=?", new String[] { school },
				null, null, MessageHelper.COLUMN_SERVER_UID + " desc", "1");
	...
	public void setDidRead(long id) {
		...
		values.put(MessageHelper.COLUMN_DID_READ, "1");
